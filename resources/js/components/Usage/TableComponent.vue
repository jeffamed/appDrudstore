<template>
    <table class="table table-bordered table-striped table-sm">
        <thead>
        <tr>
            <th  v-for="(item,index) in header" :key="index" :class="item === 'Opciones' ? 'text-center' : ''">{{ item }}</th>
        </tr>
        </thead>
        <tbody>
...
export default {
    ...
    setup(props, context)
    {
        ...
        const btnUpdate = computed(() => {return permissions.includes('usage.update')})
        const btnDelete = computed(() => {return permissions.includes('usage.delete')})
        const load = (data) => {
