...
export default {
    ...
    props: {
        ...
        header:{
            type: Array,
            default: ['Ruc','Nombre', 'Dirección','Teléfono','Opciones']
        }
    },
    setup(props, context)
    {
        const permissions = localStorage.getItem('permissions');
        const btnUpdate = computed(() => {return permissions.includes('supplier.update')})
        const btnDelete = computed(() => {return permissions.includes('supplier.delete')})
        const load = (data) => {
            context.emit('load', data);
        }

        return { load, btnUpdate, btnDelete };
    }
}
</script>
