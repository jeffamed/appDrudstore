...
<table class="tabla" id="orders">
    ...
    @foreach ($orders as $order)
        <tr style="padding-bottom: 10px; padding-top: 10px">
            <td>{{ $order->mes }}</td>
            <td style="text-align: center">{{ number_format($order->subTotal , 2) }}</td>
            <td style="text-align: center">{{ number_format($order->Descuento, 2) }}</td>
    ...
    <tfoot>
        <tr class="bold">
            ...
            <td style="text-align: center">
                {{number_format( $orders_total[0]->subTotal, 2) }}
            </td>
            <td style="text-align: center">
                {{number_format( $orders_total[0]->Descuento, 2) }}
            </td>
            <td style="text-align: center">
                {{number_format( $orders_total[0]->Total, 2) }}
            </td>
        </tr>
    </tfoot>
</table>
